...
public sealed class MainForm : Form
{
    ...
    private List<WindowInfo> GetTaskbarElements()
    {
        List<WindowInfo> result = new();

        // Работаем только с основной панелью задач.
        // Вторую панель на дополнительных мониторах не используем.
        IntPtr taskbar =
            FindWindow(
                "Shell_TrayWnd",
                null);
        ...
        // На основной панели задач находим именно список
        // работающих приложений MSTaskListWClass.
        IntPtr taskList = IntPtr.Zero;
        ...
        // Перечисляем только дочерние элементы основного
        // MSTaskListWClass. Сам контейнер в результат не добавляем.
        EnumChildWindows(
            taskList,
            (hWnd, _) =>
            {
    ...
    private async Task TestLmViewer()
    {
        ...
        {
            Log("");
            LogRound("ДИАГНОСТИКА ПАНЕЛИ ЗАДАЧ — LM VIEWER");

            List<WindowInfo> elements = GetTaskbarElements();

            Log($"Найдено элементов панели задач: {elements.Count}");

            if (elements.Count == 0)
            {
                Log("Панель задач Shell_TrayWnd не найдена или у неё нет дочерних элементов.");

                MessageBox.Show(
                    "Не удалось получить элементы панели задач.\n\n" +
                    "LM Viewer пока не нажимаем.",
                    "LM Viewer",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Warning);
            ...
            foreach (WindowInfo element in elements)
            {
                index++;

                string title = string.IsNullOrWhiteSpace(element.Title)
                    ? "<без названия>"
                    : element.Title;

                string className = string.IsNullOrWhiteSpace(element.ClassName)
                    ? "<без класса>"
                    : element.ClassName;

                string position = "?";

                if (GetWindowRect(element.Handle, out RECT rect))
                {
                    position =
                ...
                Log(
                    $"[{index}] HWND={element.Handle} | " +
                    $"Title=\"{title}\" | " +
                    $"Class=\"{className}\" | " +
                    $"PID={element.ProcessId} | {position}");
            }
            ...
            MessageBox.Show(
                "Диагностика панели задач завершена.\n\n" +
                "Посмотри журнал — там перечислены элементы панели задач.\n" +
                "Средний клик пока не выполнялся.",
                "LM Viewer",
                MessageBoxButtons.OK,
                MessageBoxIcon.Information);
        }
        ...
        {
            Log("Ошибка диагностики LM Viewer:");
            Log(ex.ToString());

            MessageBox.Show(
                "Ошибка диагностики LM Viewer:\n\n" + ex.Message,
                "LM Viewer",
                MessageBoxButtons.OK,
                MessageBoxIcon.Error);
        }
