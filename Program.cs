...
public sealed class MainForm : Form
{
    ...
    private async Task BuildFinalReports()
    {
        ...
        try
        {
            ...
            string productionDestination = FindExcelFileByStem(productionFolder, productionDestinationStem);
            string damateDestination = FindExcelFileByStem(damateFolder, damateDestinationStem);
            string bunkerDestination = FindExcelFileByStem(bunkerFolder, bunkerDestinationStem);
            ...
            await Task.Run(() =>
            {
                BuildProductionReport(productionDestination, yesterday, token);
                CheckCancellation(token);
                BuildDamateReport(damateDestination, yesterday, token);
                CheckCancellation(token);
                BuildBunkerReport(bunkerDestination, today, token);
            }, token);
        ...
        catch (Exception ex)
        {
            ...
            Log(ex.ToString());
            MessageBox.Show(ex.Message, "Ошибка сборки отчётов", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
        finally
        {
            exportCancellation?.Dispose();
...
{folder}");
    ...
    private void BuildProductionReport(string destinationFile, DateTime reportDate, CancellationToken token)
    {
        LogRound("ПРОИЗВОДСТВЕННЫЙ ОТЧЁТ");
        ReportTransfer[] transfers =
        {
            new ReportTransfer { Screen = "Катковка", SourceStem = $"{reportDate:yyyy_MM_dd} Катковка Производственный отчет", SourceRange = "D2:I37", DestinationRange = "D6:I41" },
            new ReportTransfer { Screen = "Р18", SourceStem = $"{reportDate:yyyy_MM_dd} Р18 Производственный отчет", SourceRange = "D2:I33", DestinationRange = "D42:I73" },
            new ReportTransfer { Screen = "Н-33, Н-34", SourceStem = $"{reportDate:yyyy_MM_dd} Н-33, Н-34 Производственный отчет", SourceRange = "D2:I33", DestinationRange = "D74:I105" },
            new ReportTransfer { Screen = "Р20", SourceStem = $"{reportDate:yyyy_MM_dd} Р20 Производственный отчет", SourceRange = "D2:I33", DestinationRange = "D106:I137" }
        };
        using XLWorkbook destinationBook = new XLWorkbook(destinationFile);
        IXLWorksheet destinationSheet = destinationBook.Worksheets.First();
        foreach (ReportTransfer transfer in transfers)
        {
            CheckCancellation(token);
            string sourceFile = FindExcelFileByStem(BuilderSourceFolder, transfer.SourceStem);
            ...
            using XLWorkbook sourceBook = new XLWorkbook(sourceFile);
            CopyValues(sourceBook.Worksheets.First(), transfer.SourceRange, destinationSheet, transfer.DestinationRange);
        }
        CheckCancellation(token);
    ...
    private void BuildDamateReport(string destinationFile, DateTime reportDate, CancellationToken token)
    {
        LogRound("ДАМАТЕ КЛИК");
        ReportTransfer[] transfers =
        {
            new ReportTransfer { Screen = "Катковка", SourceStem = $"{reportDate:yyyy_MM_dd} Катковка Дамате клик", SourceRange = "D2:N37", DestinationRange = "D6:N41" },
            new ReportTransfer { Screen = "Р18", SourceStem = $"{reportDate:yyyy_MM_dd} Р18 Дамате клик", SourceRange = "D2:N33", DestinationRange = "D42:N73" },
            new ReportTransfer { Screen = "Н-33, Н-34", SourceStem = $"{reportDate:yyyy_MM_dd} Н-33, Н-34 Дамате клик", SourceRange = "D2:N33", DestinationRange = "D74:N105" },
            new ReportTransfer { Screen = "Р20", SourceStem = $"{reportDate:yyyy_MM_dd} Р20 Дамате клик", SourceRange = "D2:N33", DestinationRange = "D106:N137" }
        };
        using XLWorkbook destinationBook = new XLWorkbook(destinationFile);
        IXLWorksheet destinationSheet = destinationBook.Worksheets.First();
        foreach (ReportTransfer transfer in transfers)
    ...
    private void BuildBunkerReport(string destinationFile, DateTime reportDate, CancellationToken token)
    {
        LogRound("ОСТАТОК КОРМА");
        ReportTransfer[] transfers =
        {
            new ReportTransfer { Screen = "Катковка", SourceStem = $"{reportDate:yyyy_MM_dd} Катковка Остаток корма на 00-00", SourceRange = "D2:F37", DestinationRange = "D6:G41" },
            new ReportTransfer { Screen = "Р18", SourceStem = $"{reportDate:yyyy_MM_dd} Р18 Остаток корма на 00-00", SourceRange = "D2:F33", DestinationRange = "D42:G73" },
            new ReportTransfer { Screen = "Н-33, Н-34", SourceStem = $"{reportDate:yyyy_MM_dd} Н-33, Н-34 Остаток корма на 00-00", SourceRange = "D2:F33", DestinationRange = "D74:G105" },
            new ReportTransfer { Screen = "Р20", SourceStem = $"{reportDate:yyyy_MM_dd} Р20 Остаток корма на 00-00", SourceRange = "D2:F33", DestinationRange = "D106:G137" }
        };
        using XLWorkbook destinationBook = new XLWorkbook(destinationFile);
        IXLWorksheet destinationSheet = destinationBook.Worksheets.First();
        foreach (ReportTransfer transfer in transfers)
        {
            ...
            Log($"[{transfer.Screen}] {transfer.SourceRange} → {transfer.DestinationRange}");
            using XLWorkbook sourceBook = new XLWorkbook(sourceFile);
            CopyBunkerValues(sourceBook.Worksheets.First(), destinationSheet, transfer.SourceRange, transfer.DestinationRange);
        }
        CheckCancellation(token);
        destinationBook.Save();
        Log($"Сохранён: {Path.GetFileName(destinationFile)}");
    ...
    private static string FindExcelFileByStem(string folder, string stem)
    {
        string[] files = Directory.GetFiles(folder, "*.xlsx", SearchOption.TopDirectoryOnly);
        string? match = files.FirstOrDefault(file =>
            string.Equals(Path.GetFileNameWithoutExtension(file), stem, StringComparison.OrdinalIgnoreCase));
        if (match == null)
        {
...
{folder}");
    ...
    private static void CopyValues(IXLWorksheet sourceSheet, string sourceRangeAddress, IXLWorksheet destinationSheet, string destinationRangeAddress)
    {
        IXLRange sourceRange = sourceSheet.Range(sourceRangeAddress);
        IXLRange destinationRange = destinationSheet.Range(destinationRangeAddress);
        if (sourceRange.RowCount() != destinationRange.RowCount() || sourceRange.ColumnCount() != destinationRange.ColumnCount())
        {
            throw new InvalidOperationException(
                $"Размеры диапазонов не совпадают: {sourceRangeAddress} → {destinationRangeAddress}.");
        }
        for (int row = 1; row <= sourceRange.RowCount(); row++)
        {
            for (int column = 1; column <= sourceRange.ColumnCount(); column++)
            {
                destinationRange.Cell(row, column).Value = sourceRange.Cell(row, column).Value;
            }
        }
    }

    private static void CopyBunkerValues(IXLWorksheet sourceSheet, IXLWorksheet destinationSheet, string sourceRangeAddress, string destinationRangeAddress)
    {
        IXLRange sourceRange = sourceSheet.Range(sourceRangeAddress);
        IXLRange destinationRange = destinationSheet.Range(destinationRangeAddress);
        if (sourceRange.RowCount() != destinationRange.RowCount() || sourceRange.ColumnCount() != 3 || destinationRange.ColumnCount() != 4)
        {
            throw new InvalidOperationException(
        ...
        for (int row = 1; row <= sourceRange.RowCount(); row++)
        {
            destinationRange.Cell(row, 1).Value = sourceRange.Cell(row, 1).Value;
            destinationRange.Cell(row, 2).Value = sourceRange.Cell(row, 2).Value;
            // D → D, E → E, F → G. Столбец F назначения не трогаем.
            destinationRange.Cell(row, 4).Value = sourceRange.Cell(row, 3).Value;
        }
    }
