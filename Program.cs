...
public sealed class MainForm : Form
{
    ...
    private Point fixedFilePoint = Point.Empty;
    private Point fixedExportPoint = Point.Empty;
    private Point fixedReturnPoint = Point.Empty;
    ...
    private void SaveFixedBfnCoordinates()
    {
        try
        {
            FixedBfnCoordinates data = new FixedBfnCoordinates
            {
                File = fixedFilePoint,
                Export = fixedExportPoint,
                Return = fixedReturnPoint
            };
            File.WriteAllText(
        ...
        {
            Log("Ошибка сохранения фиксированных координат BFNM: " + ex.Message);
        }
    }
    ...
    private void LoadFixedBfnCoordinates()
    {
        try
        {
            ...
            {
                FixedBfnCoordinates? data = JsonSerializer.Deserialize<FixedBfnCoordinates>(File.ReadAllText(FixedBfnSettingsFile));
                if (data != null &&
                    data.File != Point.Empty &&
                    data.Export != Point.Empty &&
                    data.Return != Point.Empty)
                {
                    fixedFilePoint = data.File;
                    fixedExportPoint = data.Export;
                    fixedReturnPoint = data.Return;
                    Log($"Фиксированные BFNM загружены: Файл=({fixedFilePoint.X},{fixedFilePoint.Y}), Экспорт=({fixedExportPoint.X},{fixedExportPoint.Y}).");
                    return;
                }
            }
            ...
            foreach (string screen in screenNames)
            {
                ...
                {
                    fixedFilePoint = filePoint;
                    fixedExportPoint = exportPoint;
                    fixedReturnPoint = returnPoint;
                    SaveFixedBfnCoordinates();
                    Log("Фиксированные координаты BFNM перенесены из старых настроек.");
                    return;
                }
            }
            Log("Фиксированные координаты «Файл» и «Экспорт» пока не найдены.");
        }
        catch (Exception ex)
        {
            Log("Ошибка загрузки фиксированных координат BFNM: " + ex.Message);
        }
    }
    ...
    private void SaveReport(string screen, string fileName, CancellationToken token)
    {
        ...
        if (fixedFilePoint == Point.Empty ||
            fixedExportPoint == Point.Empty)
        {
            throw new Exception("Не настроены фиксированные координаты BFNM «Файл» и «Экспорт».");
        }

        Log("");
        Log($"[{screen}] Сохраняем: {fileName}");
        ClickFixedPoint("Файл", fixedFilePoint, token);
        Wait(300, token);
        ClickFixedPoint("Экспорт", fixedExportPoint, token);
        Wait(1200, token);
        ...
        CancellationToken token)
    {
        ...
        if (fixedReturnPoint == Point.Empty)
        {
            throw new Exception("Не найдена фиксированная координата «Вернуться».");
        }
