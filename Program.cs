...
public sealed class MainForm : Form
{
    ...
    {
        // LM Viewer может быть свернут и тогда его главное окно
        // не попадает в обычный EnumWindows/IsWindowVisible поиск.
        // Поэтому здесь ищем именно кнопку приложения на панели задач.
        List<WindowInfo> found =
            new();
        ...
        EnumWindows(
            (hWnd, _) =>
            {
                ...
                // Основная панель задач Windows.
                if (taskbarClass.Equals(
                    "Shell_TrayWnd",
                    StringComparison.OrdinalIgnoreCase))
                {
                    SearchTaskbarChildren(
                        hWnd,
                        found);
                }

                // Панель задач на дополнительных мониторах.
                if (taskbarClass.Equals(
        ...
        foreach (WindowInfo window in found)
        {
            if (IsLmViewerWindow(window))
            {
                return window;
            }
        }
    ...
    private void SearchTaskbarChildren(
        ...
        List<WindowInfo> result)
    {
        EnumChildWindows(
            ...
            (hWnd, _) =>
            {
                ...
                // EnumChildWindows сам проходит дочерние окна,
                // поэтому здесь ничего дополнительно не запускаем.
                return true;
            },
            IntPtr.Zero);
    }
    ...
    private async Task TestLmViewer()
    {
        ...
        try
        {
            ...
            LogRound(
                "ПОИСК LM VIEWER НА ПАНЕЛИ ЗАДАЧ");

            Log(
                "Ищу именно значок LM Viewer на панели задач.");
            ...
            {
                Log(
                    "LM Viewer на панели задач не найден.");

                MessageBox.Show(
                    "Не удалось найти значок LM Viewer на панели задач.\n\n" +
                    "Убедись, что LM Viewer запущен и его значок виден на панели задач.",
                    "LM Viewer",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Warning);
            ...
            {
                Log(
                    "LM Viewer найден, но координаты значка получить не удалось.");
            ...
            Log(
                "LM Viewer НАЙДЕН НА ПАНЕЛИ ЗАДАЧ.");
            ...
            Log(
                $"Область значка: X={rect.Left}, Y={rect.Top}, " +
                $"Right={rect.Right}, Bottom={rect.Bottom}");

            Log(
                $"Центр значка: X={centerX}, Y={centerY}");

            MessageBox.Show(
                "LM Viewer найден на панели задач.\n\n" +
                $"Название: {lmViewer.Title}\n" +
                $"Класс: {lmViewer.ClassName}\n" +
                $"Центр: X={centerX}, Y={centerY}\n\n" +
                "Средний клик пока НЕ выполняется.",
                "LM Viewer — тест поиска",
                MessageBoxButtons.OK,
                MessageBoxIcon.Information);
        }
        catch (Exception ex)
