...
public sealed class MainForm : Form
{
        ...
        CancellationToken token)
    {
        ...
        try
        {
            ...
            // Пересчитать формулы общего отчёта после вставки F → G.
            destinationBook.ForceFormulaRecalculation = true;
        ...
        string destinationRangeAddress)
    {
        ...
        for (int rowOffset = 0; rowOffset < sourceRows; rowOffset++)
        {
            ...
            ICell destinationF = GetOrCreateCell(
                destinationSheet,
                destinationRange.FirstRow + rowOffset,
                destinationRange.FirstColumn + 2);

            // D → D, E → E, F → F.
            // Столбец G назначения не изменяем.
            CopyCellValue(sourceD, destinationD, evaluator);
            CopyCellValue(sourceE, destinationE, evaluator);
            CopyNumericValueOnly(sourceF, destinationF);
        }
    }
        ...
        ICell destination)
    {
        ...
        // В выгружаемом отчёте F содержит обычное значение, без формулы.
        // Переносим только это значение в G. Формулу и оформление
        // исходной ячейки не копируем. Форматирование G остаётся
        // тем, которое уже есть в общем отчёте.
        switch (source.CellType)
        {
            case CellType.Numeric:
                destination.SetCellValue(source.NumericCellValue);
            ...
            case CellType.String:
                string text = source.StringCellValue?.Trim() ?? "";
                ...
                if (double.TryParse(
                    ...
                    out numericValue))
                {
                    destination.SetCellValue(numericValue);
                }
                else
                {
                    destination.SetCellValue(text);
                }
                break;
