...
public sealed class MainForm : Form
{
    ...
    // Фиксированные кнопки BFNM хранятся отдельно для каждого из 4 экранов.
    private readonly Dictionary<string, FixedBfnCoordinates> fixedBfnByScreen = new();

    private string FixedBfnSettingsFile =>
        Path.Combine(SettingsFolder, "fixed-bfn-coordinates.json");
    ...
    public MainForm()
    {
        ...
        lmViewerTestButton.Click +=
            (_, _) =>
            {
                Log("LM Viewer: автоматический поиск отключён. Настройку координат добавим отдельно.");
            };
        ...
        Panel leftPanel = new Panel { Dock = DockStyle.Left, Width = 300, Padding = new Padding(8) };
        Panel rightPanel = new Panel { Dock = DockStyle.Fill, Padding = new Padding(8) };

        Panel infoPanel = new Panel { Dock = DockStyle.Top, Height = 135 };
        instruction.Dock = DockStyle.Top; instruction.Height = 42;
        instruction.Font = new Font("Segoe UI", 9); instruction.TextAlign = ContentAlignment.MiddleCenter;
        mousePosition.Dock = DockStyle.Top; mousePosition.Height = 26;
        mousePosition.Font = new Font("Segoe UI", 8); mousePosition.TextAlign = ContentAlignment.MiddleCenter;
        currentScreenLabel.Dock = DockStyle.Top; currentScreenLabel.Height = 26;
        currentScreenLabel.Font = new Font("Segoe UI", 9, FontStyle.Bold); currentScreenLabel.TextAlign = ContentAlignment.MiddleCenter;
        screenSelector.Dock = DockStyle.Top; screenSelector.Height = 30;
        infoPanel.Controls.Add(screenSelector); infoPanel.Controls.Add(currentScreenLabel); infoPanel.Controls.Add(mousePosition); infoPanel.Controls.Add(instruction);

        TableLayoutPanel reportButtons = new TableLayoutPanel { Dock = DockStyle.Top, Height = 125, ColumnCount = 2, RowCount = 2, Padding = new Padding(2) };
        reportButtons.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 50)); reportButtons.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 50));
        reportButtons.RowStyles.Add(new RowStyle(SizeType.Absolute, 58)); reportButtons.RowStyles.Add(new RowStyle(SizeType.Absolute, 58));
        reportButtons.Controls.Add(exportScreen1Button, 0, 0); reportButtons.Controls.Add(exportScreen2Button, 1, 0);
        reportButtons.Controls.Add(exportScreen3Button, 0, 1); reportButtons.Controls.Add(exportScreen4Button, 1, 1);
        foreach (Control c in reportButtons.Controls) { c.Margin = new Padding(3); c.Font = new Font("Segoe UI", 9, FontStyle.Bold); c.Dock = DockStyle.Fill; }

        exportButton.Dock = DockStyle.Top; exportButton.Height = 55; exportButton.Margin = new Padding(3, 8, 3, 8);

        TableLayoutPanel tools = new TableLayoutPanel { Dock = DockStyle.Top, Height = 110, ColumnCount = 2, RowCount = 2, Padding = new Padding(2) };
        tools.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 50)); tools.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 50));
        tools.RowStyles.Add(new RowStyle(SizeType.Absolute, 52)); tools.RowStyles.Add(new RowStyle(SizeType.Absolute, 52));
        tools.Controls.Add(setupButton, 0, 0); tools.Controls.Add(lmViewerTestButton, 1, 0); tools.Controls.Add(foldersButton, 0, 1); tools.Controls.Add(transferButton, 1, 1);
        foreach (Control c in tools.Controls) { c.Margin = new Padding(3); c.Dock = DockStyle.Fill; }

        cancelButton.Dock = DockStyle.Top; cancelButton.Height = 38; cancelButton.Margin = new Padding(3, 8, 3, 3);
        leftPanel.Controls.Add(cancelButton); leftPanel.Controls.Add(tools); leftPanel.Controls.Add(reportButtons); leftPanel.Controls.Add(exportButton); leftPanel.Controls.Add(infoPanel);
        rightPanel.Controls.Add(log);
        Controls.Add(rightPanel); Controls.Add(leftPanel);
    ...
    private void CapturePoint()
    {
        ...
        if (setupIndex >= pointNames.Length)
        {
            ...
            instruction.Text =
                $"Готово! 3 координаты для «{screen}» сохранены.";

            Log(
                $"Все 3 координаты для «{screen}» сохранены.");
    ...
    // =========================================================
    // FIXED BFNM COORDINATES
    // =========================================================

    private sealed class FixedBfnCoordinates
    {
        public Point File { get; set; }
        public Point Export { get; set; }
        public Point Return { get; set; }
    }

    private void SaveFixedBfnCoordinates()
    {
        try
        {
            File.WriteAllText(
                FixedBfnSettingsFile,
        ...
        {
            Log(
                "Ошибка сохранения фиксированных координат BFNM: " +
                ex.Message);
        }
    }

    private void LoadFixedBfnCoordinates()
    {
        try
        {
            fixedBfnByScreen.Clear();

            // Сначала пытаемся загрузить новую структуру:
            // отдельные координаты Файл/Экспорт/Вернуться для каждого экрана.
            if (File.Exists(FixedBfnSettingsFile))
            {
                string json =
                    File.ReadAllText(FixedBfnSettingsFile);

                Dictionary<string, FixedBfnCoordinates>? loaded =
                    JsonSerializer.Deserialize<Dictionary<string, FixedBfnCoordinates>>(json);

                if (loaded != null)
                {
                    foreach (string screen in screenNames)
                    {
                        if (loaded.TryGetValue(screen, out FixedBfnCoordinates? data) &&
                            data.File != Point.Empty &&
                            data.Export != Point.Empty &&
                            data.Return != Point.Empty)
                        {
                            fixedBfnByScreen[screen] = data;
                        }
                    }
                }
            }

            // Добираем координаты из старого coordinates.json.
            // Это важно: уже сохранённые привязки пользователей не теряются.
            foreach (string screen in screenNames)
            {
                if (fixedBfnByScreen.ContainsKey(screen))
                {
                    continue;
                }
                ...
                if (allPoints.TryGetValue(
                        screen,
                        out Dictionary<string, Point>? points) &&
                    points.TryGetValue("Файл", out Point filePoint) &&
                    points.TryGetValue("Экспорт", out Point exportPoint) &&
                    points.TryGetValue("Вернуться", out Point returnPoint) &&
                    filePoint != Point.Empty &&
                    exportPoint != Point.Empty &&
                    returnPoint != Point.Empty)
                {
                    fixedBfnByScreen[screen] =
                        new FixedBfnCoordinates
                        {
                            File = filePoint,
                            Export = exportPoint,
                            Return = returnPoint
            ...
            SaveFixedBfnCoordinates();

            foreach (string screen in screenNames)
            {
                if (fixedBfnByScreen.TryGetValue(
                        screen,
                        out FixedBfnCoordinates? data))
                {
                    Log(
                        $"{screen}: Файл=({data.File.X},{data.File.Y}), " +
                        $"Экспорт=({data.Export.X},{data.Export.Y}), " +
                        $"Вернуться=({data.Return.X},{data.Return.Y})");
                }
                else
                {
                    Log(
                        $"{screen}: фиксированные координаты BFNM не найдены.");
                }
            }
        }
        catch (Exception ex)
        {
            Log(
                "Ошибка загрузки фиксированных координат BFNM: " +
                ex.Message);
        }
    }

    private FixedBfnCoordinates GetFixedBfnCoordinates(
        string screen)
    {
        if (!fixedBfnByScreen.TryGetValue(
                screen,
        ...
        {
            throw new Exception(
                $"Для экрана «{screen}» не настроены фиксированные координаты BFNM «Файл», «Экспорт» и «Вернуться».");
        }
    ...
    private void SaveReport(string screen, string fileName, CancellationToken token)
    {
        ...
        FixedBfnCoordinates fixedPoints =
            GetFixedBfnCoordinates(screen);

        Log("");
        Log($"[{screen}] Сохраняем: {fileName}");

        // ВАЖНО: Файл и Экспорт берём именно для текущего экрана/монитора.
        ClickFixedPoint(
            $"Файл [{screen}]",
            fixedPoints.File,
            token);
        ...
        Log("Вводим новое имя файла.");
        SendKeys.SendWait(fileName);
        Wait(300, token);

        // В окне сохранения новое имя уже выделено/готово к вводу.
        // Enter = сохранить. Затем Tab переводит фокус на «Нет», Enter подтверждает.
    ...
    private void ClickFixedPoint(string name, Point p, CancellationToken token)
    {
        CheckCancellation(token);
        Log($"Фиксированный клик: {name} ({p.X},{p.Y})");
        Cursor.Position = p;
        Wait(250, token);
    ...
    private void ValidateAllCoordinates()
    {
        foreach (string screen in screenNames)
        {
            ...
            {
                throw new Exception(
                    $"Для экрана «{screen}» не настроены 3 координаты отчётов.");
            }
        }
    }
