...
public sealed class MainForm : Form
{
    ...
    private List<WindowInfo> GetTaskbarElements()
    {
        ...
        void AddWindow(IntPtr hWnd, string source)
        {
            if (hWnd == IntPtr.Zero || !IsWindow(hWnd) || !visited.Add(hWnd))
            {
                return;
            }
        ...
        {
            AddWindow(taskbar, "Taskbar");

            EnumChildWindows(
                taskbar,
                (hWnd, _) =>
                {
                    AddWindow(hWnd, "Taskbar child");
                    return true;
                },
                IntPtr.Zero);
        }
        ...
        {
            AddWindow(secondaryTaskbar, "Secondary taskbar");

            EnumChildWindows(
                secondaryTaskbar,
                (hWnd, _) =>
                {
                    AddWindow(hWnd, "Secondary taskbar child");
                    return true;
                },
                IntPtr.Zero);
        }
